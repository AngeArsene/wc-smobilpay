 ...
 * Plugin URI: https://github.com/AngeArsene/wc-smobilpay
...
if (! function_exists('wc_smobilpay_init')) {
    /**
     * Initializes the Smobilpay payment gateways for WooCommerce.
     *
     * This function checks if the WooCommerce payment gateway base class exists,
     * includes the necessary Smobilpay gateway classes, and registers them
     * with WooCommerce via the `woocommerce_payment_gateways` filter.
     *
     * @return void
     */
...
if (! function_exists('wc_smobilpay_add_gateways')) {
...
if (! function_exists('wc_smobilpay_handle_webhook')) {
    function wc_smobilpay_handle_webhook()
    {
        ...
        if (isset($_SERVER['HTTP_X_WC_WEBHOOK_SIGNATURE'])) {
            ...
            $calculated = base64_encode(hash_hmac('sha256', $raw_post, $secret, true));
        } else if (isset($_SERVER['HTTP_X_SIGNATURE'])) {
            $signature = $_SERVER['HTTP_X_SIGNATURE'];
            $calculated = hash_hmac('sha1', $raw_post, $secret);
        ...
        if ($gateway && method_exists($gateway, 'verify_transaction')) {
            ...
            if ($trid) {
                ...
                if (!empty($result)) {
                    error_log('Smobilpay Webhook Verification Result: ' . json_encode($result, JSON_PRETTY_PRINT));
                }
...
if (! function_exists('whatsapp_notify_client')) {
    function whatsapp_notify_client($order, $errorMessages)
    {
...
if (! function_exists('email_notify_client')) {
    function email_notify_client($order, $errorMessages)
    {
        /** @var \WC_Order  $order */
        /** @var array $errorMessages */
...
if (! function_exists('wc_smobilpay_get_site_logo')) {
